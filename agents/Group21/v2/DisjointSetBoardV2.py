...
class DisjointSetBoardV2:
    ...
    @classmethod
    def from_existing_board(cls, board: Board) -> 'DisjointSetBoardV2':
        """Converts a Board to a DisjointSetBoard."""
        dsu_board = cls()
        for r in range(DisjointSetBoardV2.N):
            for c in range(DisjointSetBoardV2.N):
                if board.tiles[r][c].colour is not None:
                    index = r * DisjointSetBoardV2.N + c
...
DisjointSetBoardV2.NEIGHBOURS = [
    [
        r * DisjointSetBoardV2.N + c
        for dr, dc in DisjointSetBoardV2.NEIGHBOUR_OFFSETS
        for (r, c) in [(index // DisjointSetBoardV2.N + dr, index % DisjointSetBoardV2.N + dc)]
        if 0 <= r < DisjointSetBoardV2.N and 0 <= c < DisjointSetBoardV2.N
    ]
    for index in range(DisjointSetBoardV2.SIZE)
]
