...
from agents.Group21.v2.MCTSNodeV2 import MCTSNodeV2
...
class MCTSV2:
    ...
    def __init__(self, colour: Colour):
        self.colour = colour
        self._root: MCTSNodeV2 | None = None
    ...
    def run(self, time_limit: float = 0.5, iterations: int = 2000) -> Move:
        ...
        r, c = divmod(best_move, DisjointSetBoardV2.N)
        return Move(r, c)
    ...
    def update(self, board: Board, opp_move: Move | None) -> None:
        ...
        move = (opp_move.x * DisjointSetBoardV2.N + opp_move.y) if opp_move is not None else None
        ...
        # Otherwise, create a completely new root node
            ...
            if opp_move == MCTSV2.SWAP_MOVE:
                self.colour = Colour.opposite(self.colour)
            self._root = MCTSNodeV2(self.colour, DisjointSetBoardV2.from_existing_board(board))

    def _select(self) -> MCTSNodeV2:
    ...
    @staticmethod
    def _backpropagate(node: MCTSNodeV2, reward: float, moves: list[int]) -> None:
